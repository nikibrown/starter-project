...
- `npm run dev` to run the Vite dev server (edit files under `src/`)
- The dev server URL is printed in the terminal (default [http://localhost:5173](http://localhost:5173))
- `npm run build` outputs the static site to the `/docs` folder (for GitHub Pages)
- `npm run preview` serves the production build locally
- Static assets that should be copied as-is (favicons, etc.) go in `/public`
